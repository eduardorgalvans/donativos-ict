...
@section('contenido')
        <div class="panel panel-inverse" data-sortable-id="form-stuff-11">
            ...
            <div class="panel-body">
                {!! Form::open(["route"=>'admin.permanentes.store', "method"=>"POST", "class"=>"form-horizontal", "role"=>"form" ]) !!}
                    <fieldset>
                        <h4 class="mb-3">Agregar <small>un nodo al menú del sistema.</small></h4>
                        @include('layouts.request')
                        @include('admin.permanentes.forms.form') {{-- admin/permanentes/forms/form --}}
                        <hr>
                        <div class="row">
                            <div class="col-md-8 offset-md-2">
                                {!! Form::submit( 'Guardar', [ 'class'=>'btn btn-primary w-100px me-5px', ] ) !!}
                                {!! Html::decode( link_to_route( 'admin.permanentes.index', 'Cancelar', null, [ "class"=>"btn btn-default w-100px", ] ) ) !!}
                            </div>
                        </div>
                    </fieldset>
                {!! Form::close() !!}
